...
真实执行按 `data/pharm/<方名>/01_batman/`、`02_disease/`、`03_intersect/`、`04_ppi/`、`05_enrich/` 沉淀，每层为 `run_<run_id>/attempt_<nn>/`。每步结束即保存，partial/blocked 也保留访问证据；状态必须明确，目录存在不代表分析通过。fixture 禁止进入。
...
流程图使用 8 条依赖边：规划到两路靶点，两路靶点到交集，交集到两路分析，两路分析到协调验收。节点位置改变时连线重新计算。
