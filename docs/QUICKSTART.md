...
六种角色由独立 Codex CLI 会话执行，默认使用 `icrc`、`gpt-5.6-luna`、`medium`，执行设置见 `configs/runtime.json`。协调角色分别进行规划和验收，因此一次完整新运行包含七个模型会话、九个阶段；GeneCards 与 OMIM 独立并行，疾病靶点由程序合并，交集由 Playwright 实际操作 Venny 2.1.0 并用 Python 独立核对，两个工具步骤均不增加模型会话。
...
左侧“新建任务”可直接填写研究名称、药材、疾病关键词和研究说明。点击“保存并启动”会将任务写入 `tasks/tasks.local.jsonl` 并启动现有多 Agent 执行器；“保存任务”仅加入清单。相同内容重复保存会复用原任务，改动已有内容保存为新的任务，不改写历史运行快照。
...
新合成演练也需要模型服务、Playwright Chromium 和 Venny 网站可用；自动单元测试会替换远端调用，不能把单元测试通过等同于线上演练成功。已导出的 HTML 报告可离线打开。
...
当前上游自动执行尚未完成“网页全量导出 → 合格导入”的闭环。补充账号只是访问条件之一；完整导出、来源台账、映射与参数也需准备。完整进度见 [项目状态](PROJECT_STATUS.md)。
...
页面可一键填入五类甲状腺疾病。GeneCards 合并所选疾病的全部完整检索记录后统一计算中位数，严格筛选高于中位数的记录，再与 OMIM 靶点合并去重。跨疾病分数保留，不先按基因选最大分数或求平均。规则标记待医生确认；fixture 仍使用固定测试集合。

新流程显示 GeneCards、OMIM 和疾病靶点合并三个节点。历史版本运行继续显示旧图；要使用新流程，应从任务新建一次运行。

交集现在实际访问官方 Venny 2.1.0，测试模式也需要网络与已安装的 Playwright Chromium。点击“共同靶点交集”可下载原图、结果文本与访问记录；方法与异常处理见 [Venny 接入说明](VENNY.md)。
