...
每次运行创建全新会话，前景和背景处于同一会话中；不复用浏览器个人会话。进程内请求间隔至少 10 秒，提交不自动重试，访问限制不会绕过。列表大小由配置 `max_list_size` 控制（默认 400，上限 3000）；2026-09-19 实测 659 个共同靶点一次性提交成功（659/659 映射，7,558 条返回、1,232 条 BH<0.05）。物种背景由 DAVID 提供，不受本地上传数量限制。
