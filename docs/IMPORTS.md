...
当前真实网络分支可调用 STRING 公开 API，或显式配置本地 v12.0 文件离线构建；CytoNCA 桥接（非加权 Degree）与 DAVID 正式导出均已接入，
NetworkX 与本地合成统计产物仍分别标注实际方法，不冒充 CytoNCA/DAVID。正式拓扑指标口径、DAVID 背景集与统计方法确认仍是待完成项，已有界面和角色不代表研究参数已确认。
