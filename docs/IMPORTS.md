...
当前真实网络分支优先读取配置的 STRING 12.0 本地物种文件，未配置时调用 STRING 公开 API；本地文件目录按 [环境准备](ENVIRONMENT_SETUP.md#string-120-本地数据) 设置。NetworkX 度值会明确标注方法，不能冒充 CytoNCA。DAVID 正式导出、背景集确认及 CytoNCA 接入仍是待完成项，已有界面和角色不代表这些分析已完成。
...
在 `tasks/tasks.local.jsonl` 该任务增加 `"import_batch":"20260913_01"` 即明确选用该批次；不猜测“最新文件夹”。不配置时使用旧入口。每次补充/修订新建批次，`raw/` 原始导出保持不变。阶段归档为不可变历史，不能直接把历史归档当作可编辑导入区。
