...
def export(run_id):
    ...
    intro = "本报告的靶点集合与候选疾病关联均为固定合成测试集合，只用于工程验证，不是数据库查询结果。" if synthetic else "本报告保留本地数据的反查结果与受限记录。受限步骤未生成替代靶点或分析结果，后续复查以最新 attempt 为准。"
    sections = []
    ...
    document = '<!doctype html><html lang="zh-CN"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Pharmacology Multi-Agent 运行报告</title><style>' + css + '</style><main><div class="badge">' + badge + '</div><h1>Pharmacology Multi-Agent</h1><p>' + intro + '</p><p>案例：' + escape(case) + '</p><small>运行：' + escape(run_id) + '<br>更新时间：' + escape(manifest.get("updated_at", "")) + '</small><div class="metrics">' + metrics + '</div>' + ''.join(sections) + '<footer><small>离线导出；不连接模型或外部数据库。此文件只包含经过筛选的公开展示产物，不含凭据、原始模型日志或提示词。</small></footer></main></html>'
