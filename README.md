...
新运行包含六种 Agent 角色、七次独立模型会话及九个阶段，由 DAG 执行器按依赖并行调度；协调规划可在注册阶段内建议启停（程序校验，越界回退默认图），失败阶段自动有限重试，协调验收可点名返工至多一轮。GeneCards 与 OMIM 独立并行，二者通过检查后才合并；任一路受限时，另一支的成功记录可保留并在输入不变时复用。疾病靶点合并由程序执行；交集由 Playwright 操作官方 Venny 2.1.0，再由 Python 独立核对。旧版运行保留原有流程与历史记录。
