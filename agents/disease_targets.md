...
恢复旧版时同样区分合格导入与访问核验；不声称已完成全量网页导出。当前暂定 GeneCards 全部查询记录统一中位数、严格大于筛选，OMIM 不套用该分数；原文未明确的口径保持待确认。历史产物不回写；重试按当前执行器产生新 attempt，若重新执行交集则使用当前 Venny 工具，不能据此追认旧 attempt 已使用 Venny。
