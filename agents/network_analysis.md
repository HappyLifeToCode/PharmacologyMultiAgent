...
当前真实路径优先读取配置的 STRING 12.0 本地物种文件，未配置时调用 STRING API，再以 NetworkX 计算 Degree；本角色审核映射、阈值、版本、文件哈希和未识别节点。Cytoscape 3.10.0 / CytoNCA 2.1.6 已完成独立合成安装测试，但正式网络自动交接尚未实现，当前输出只能标为 NetworkX，并保留 partial 与方法缺口。后续接通实际插件、保存原始表及全部所需拓扑参数后才能据实声明使用 CytoNCA。
